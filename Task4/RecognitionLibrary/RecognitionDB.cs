namespace RecognitionLibrary
{
    ...
    public class RecognitionLibraryContext : DbContext
    {
        ...
        public void addRes(RecognitionInfo t)
        {
            
            RecognitionImage ri = new RecognitionImage();
            ri.Confidence = t.Confidence;
            ri.Path = t.Path;
            ri.Statistic = 0;
            ri.Label = Convert.ToInt32(t.Class);
            ri.ImageDetails = new Blob() { Image = t.Image };
            RecognitionImages.Add(ri);
            Blobs.Add(ri.ImageDetails);
